...
class Contents {
  ...
  addStylesheetRules(rules, key) {
    var styleEl
    var styleSheet
    var key = key || 'epubjs-inserted-css'
  ...
  processStylesheets() {
    ...
    styleSheets.forEach(styleSheet => {
      // إزالة أي قيود على تحميل ملفات CSS
      styleSheet.setAttribute('crossorigin', 'anonymous')
      
      // إضافة معالجة خاصة للأنماط المضمنة
      if (styleSheet.href && styleSheet.href.startsWith('blob:')) {
        // معالجة خاصة للأنماط المضمنة في الكتاب
        this.loadBlob(styleSheet.href)
          .then(css => {
            // تطبيق CSS مباشرة بدون تصفية
            this.addStylesheetCss(css, 'embedded-css-' + Math.random().toString(36).substring(2, 15))
          })
